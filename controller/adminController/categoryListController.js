...
const editCategoryPost = async (req, res) => {
  ...
  const edit = await Category.updateOne(
    { _id: id },
    { $set: { category: categoryName, categoryDescription } }
  );
...
const createCategoryOffer = async (req, res) => {
  try {
    ...
    if (duplicate) {
      throw new Error("this category Offer Exist ");
    }
    const newOffer = await offerCategory.create({
      categoryId,
      offerRate,
      startDate,
      expiryDate,
    ...
    if (productUpdate) {
      productUpdate.forEach(async (pro) => {
        ...
        if (
          pro.productOfferRate &&
          pro.productOfferRate > pro.categoryOfferRate
        ) {
          await product.findOneAndUpdate(
            { _id: pro._id },
            {
              $set: {
                categoryOfferRate: categoryOfferRate,
              },
            },
            { new: true }
          );
...
const deleteCategoryOffer = async (req, res) => {
  try {
    ...
    const productUpdate = await product.find({
      category: offerDetails.categoryId,
    });
    productUpdate.forEach(async (pro) => {
      if (pro.productOfferRate) {
        await product.findOneAndUpdate(
          { _id: pro._id },
          { $unset: { categoryOfferRate: "" } },
          { new: true }
        );
        const productOfferRate = pro.productOfferRate;
        const originalPrice = pro.amount;
        const offerPrice =
...
const editCategoryOffer = async (req, res) => {
  try {
    console.log(req.body);
    const { id, categoryId, offerRate, startDate, expiryDate } = req.body;
    ...
    const newOffer = await offerCategory.findOneAndUpdate(
      { _id: id },
      {
        categoryId,
        offerRate,

...
const applyCoupon = async (req, res) => {
  try {
    ...
    const currentCoupons = await user.findOne({
      _id: userId,
      allCoupon: couponData._id,
    });
    ...
    if (!existCoupon.allCoupon) {
      await user.findByIdAndUpdate(
        { _id: userId },
        { $set: { allCoupon: [couponData._id] } }
      );
    } else {
      await user.findByIdAndUpdate(
        { _id: userId },
        { $addToSet: { allCoupon: couponData._id } }
      );
    }
    const offerCart = await cart.findOneAndUpdate(
      { userData: userId },
      { $set: { couponOffer: couponData.value, couponId: couponData._id } }
    );
